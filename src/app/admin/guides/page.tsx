...
const CATEGORIES = ['debutant', 'metier', 'economie', 'ecoGnome', 'ville']
...
export default function AdminGuidesPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    const r = await fetch('/api/guides?admin=1').then(r => r.json()).catch(() => [])
    setGuides(r)
    setLoading(false)
  }, [])
    ...
    <div className="space-y-6">
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-[#111F18] border border-[rgba(82,183,136,0.15)] rounded-xl p-6 space-y-4">
          ...
          <div>
            <label className="text-xs text-[#9DC4AD] mb-1 block">Résumé</label>
            ...
            <div key={g.id} className="bg-[#111F18] border border-[rgba(82,183,136,0.1)] rounded-xl p-4 flex items-center gap-4">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-xs text-[#52B788] bg-[rgba(82,183,136,0.1)] px-2 py-0.5 rounded">{g.category}</span>
                  {g.published
