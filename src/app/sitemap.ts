...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const staticPages: MetadataRoute.Sitemap = [
    ...
    { url: `${BASE}/villes`,         lastModified: now, changeFrequency: 'weekly',  priority: 0.7 },
  ...
  try {
    const [articles, guides, events, cities, tutorials, forumPosts, giveaways] = await Promise.all([
    ...
    const dynamic: MetadataRoute.Sitemap = [
      ...
      ...cities.map(c => ({ url: `${BASE}/villes/${c.slug}`, lastModified: c.updatedAt, changeFrequency: 'weekly' as const, priority: 0.6 })),
